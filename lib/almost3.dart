...
import 'app_settings3.dart'; //страница настроек
import 'notes_screen.dart'; // панель закладок
import 'dart:io'; // Добавьте этот импорт
...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    SystemChrome.setSystemUIOverlayStyle(SystemUiOverlayStyle(
      statusBarColor: Colors.transparent, // Укажите желаемый цвет
      statusBarIconBrightness: Brightness.light, // Укажите желаемую яркость значков (черные или белые)
    ));
    return MaterialApp(
      home: WelcomeScreen(),
    );
              ...
              Container(
                decoration: BoxDecoration(
                  image: DecorationImage(
                    image: FileImage(File(AppSettings().backgroundImagePath)), // Используйте FileImage для изображения из файла
                    fit: BoxFit.cover,
                  ),
                ),
              ),
